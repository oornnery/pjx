...
class TemplateMetadata:
    ...
    computed: list[ComputedDecl] = field(default_factory=list)
    ...
    def resolve_import(self, name: str, current_file: str | None = None) -> str | None:
        for imp in self.imports:
            ...
            if source.startswith("..") or source.startswith("."):
                ...
                resolved = "/".join(dir_parts) + f"/{name}.jinja"
            else:
                module_path = source.replace(".", "/")
                resolved = f"{module_path}/{name}.jinja"

            if ".." in resolved.split("/") or resolved.startswith("/"):
                return None
            return resolved
        return None
