...
class Scanner:
    ...
    def _scan_comment(self) -> ScanToken:
        ...
        return ScanToken(ScanTokenType.COMMENT, self._source[start:end], line=line, col=col)
    ...
    def _scan_open_tag(self) -> ScanToken:
        ...
        if self._pos < len(self._source) and self._source[self._pos] == ">":
            self._advance(1)
        token_type = ScanTokenType.SELF_CLOSING_TAG if self_closing else ScanTokenType.OPEN_TAG
        return ScanToken(
            token_type,
            self._source[start : self._pos],
            tag_name=tag_name,
    ...
    def _scan_attributes(self) -> list[TagAttribute]:
        ...
        while self._pos < len(self._source):
            ...
            if ch in (">", "/"):
                break
            # Spread: ...{expr}
            if ch == "." and self._source[self._pos : self._pos + 3] == "...":
                attrs.append(self._scan_spread_attribute())
                continue
            # Conditional: ?attr={expr}
            if ch == "?":
                attrs.append(self._scan_conditional_attribute())
                continue
    ...
    def _scan_spread_attribute(self) -> TagAttribute:
        line, col = self._line, self._col
        self._advance(3)  # ...
        if self._pos < len(self._source) and self._source[self._pos] == "{":
            value, is_expr = self._read_expression_value()
            return TagAttribute("...", None, value, is_expr, line, col, is_spread=True)
        return TagAttribute("...", None, None, False, line, col, is_spread=True)

    def _scan_conditional_attribute(self) -> TagAttribute:
        line, col = self._line, self._col
        self._advance(1)  # ?
        ...
        if self._pos < len(self._source) and self._source[self._pos] == "=":
            ...
            return TagAttribute(name, namespace, value, is_expr, line, col, is_conditional=True)
        return TagAttribute(name, namespace, None, False, line, col, is_conditional=True)
    ...
    def _scan_text(self) -> ScanToken:
        ...
        while self._pos < len(self._source):
            ...
            if self._source[self._pos] == "<" and self._peek_tag_start():
                break
            self._advance(1)
        return ScanToken(ScanTokenType.TEXT, self._source[start : self._pos], line=line, col=col)
