    ...
    def process(self, source: str, ctx: ProcessorContext) -> ProcessorResult:
        ...
        while i < len(tokens):
            ...
            if (
                ...
                and token.tag_name in imported_names
            ):
            ...
            if (
                token.type == ScanTokenType.OPEN_TAG
                and token.tag_name in imported_names
            ):
                children_tokens, end_idx = self._collect_children(
                    tokens, i, token.tag_name
                )
                ...
                nested = ComponentProcessor().process(children_source, ctx)
                children_source = nested.source
                result.append(
                    self._compile_component(token, children_source, ctx, imported_names)
                )
            ...
            if (
                token.type == ScanTokenType.CLOSE_TAG
                and token.tag_name in imported_names
            ):
                i += 1
                continue
